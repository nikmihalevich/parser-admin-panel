...
// create the connection to database
    ...
    MYSQLconnection.promise().query("SELECT product_id, name FROM " + DB_PREFIX + "product_description")
        .then((res) => {
    ...
    MYSQLconnection.promise().query("SELECT weight_class_id, unit FROM " + DB_PREFIX + "weight_class_description")
        .then((res) => {
            res[0].forEach((value, i) => {
    ...
    MYSQLconnection.promise().query("SELECT product_id, price, weight, weight_class_id FROM " + DB_PREFIX + "product")
        .then((res) => {
            res[0].forEach((value, i) => {
                let params = {
                    weight: value.weight.split('.')[0] > 1000 ? value.weight.split('.')[0] / 1000 : value.weight.split('.')[0],
        ...
        .then(() => {
            for (let i = 0; i < productsCharacteristic.length; i++) {
                for (let j = 0; j < weightClassUnit.length; j++) {
                    if(productsCharacteristic[i].params.weight_class_id === weightClassUnit[j].weight_class_id) {
                        if(productsCharacteristic[i].params.isConverted) {
                            if(weightClassUnit[j].unit === 'мл') productsCharacteristic[i].params.unit = 'л'
                            if(weightClassUnit[j].unit === 'гр') productsCharacteristic[i].params.unit = 'кг'
                        } else {
